...
<body <?php body_class(); ?>>
    ...
    <header id="masthead" class="masthead_<?php  echo ( get_field( "header_background_image", PAGE_ID_WORK ) ? "is_image" : 'is_not_image') ?>">
        
        <video autoplay="" data-automation="VideoPlayer_video_video" loop="" muted="" playsinline="" poster="<?php echo get_field( "header_background_image", PAGE_ID_WORK ); ?>">
            <?php
                if( get_field( "video_webm", PAGE_ID_WORK ) ) {
                    echo '<source src="'.get_field( "video_webm", PAGE_ID_WORK ).'" type="video/webm">';
                }
                if( get_field( "video_mp4", PAGE_ID_WORK ) ) {
                    echo '<source src="'.get_field( "video_mp4", PAGE_ID_WORK ).'" type="video/mp4">';
                }
            ?>
        </video>
        
        <?php
            if( get_field( "header_title", PAGE_ID_WORK ) || get_field( "header_subtitle", PAGE_ID_WORK ) ) {
                echo "<div class='container text-center'>";
                
                if( !is_front_page() ) {
                    echo "<h2>".get_the_title( PAGE_ID_WORK )."</h2>";
                }
                
                if( get_field( "header_title", PAGE_ID_WORK ) ) {
                    echo "<div class='header_title'>".get_field( "header_title", PAGE_ID_WORK )."</div>";
                }
                
                if( get_field( "header_subtitle", PAGE_ID_WORK ) ) {
                    echo "<div class='header_subtitle'>".get_field( "header_subtitle", PAGE_ID_WORK )."</div>";
                }
                
                if( get_field( "header_link", PAGE_ID_WORK ) ) {
                    echo "<div class='header_link'>";
                    echo "<a href='".get_field( "header_link", PAGE_ID_WORK )['url']."' ".( ( !empty( get_field( "header_link", PAGE_ID_WORK )['target'] ) ) ? ' target="_blank" ' : '' ).">".get_field( "header_link", PAGE_ID_WORK )['title']."</a>";
                    echo "</div>";
                }
