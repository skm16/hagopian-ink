...
add_filter( 'option_posts_per_page', 'change_posts_per_page' );
function change_posts_per_page( $value ) {
...
// remove active state from blog when on single work
...
function theme_get_archives_link ( $link_html ) {
  ...
  // add header scripts
  function hk_header_scripts() {
    if(get_field('header_scripts', 'options', false, false)):
      the_field('header_scripts', 'options', false, false);
    endif;
    return;
  }
  add_action('wp_head', 'hk_header_scripts');

  // add footer scripts
  function hk_footer_scripts() {
    if(get_field('footer_scripts', 'options')):
      the_field('footer_scripts', 'options');
    endif;
    return;
  }
  add_action('wp_footer', 'hk_footer_scripts');


  // add post builder padding
  function return_post_builder_padding() {
    $padding = get_sub_field('padding');
    if($padding):
      if($padding === 'default-padding'):
        $output = 'padding:2.5rem 0;';
      else:
        $output = 'padding:5rem 0;';
      endif;
      return $output;
    endif;
    return false;
